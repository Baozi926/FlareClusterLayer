...
import * as Deferer from "dojo/_base/Deferred";
...
export class FlareClusterLayer extends asd.declared(GraphicsLayer) {
  ...
  constructor(options: FlareClusterLayerProperties) {
    ...
    if (!options) {
      ...
      console.error(
        "Missing required parameters to flare cluster layer constructor."
      );
      return;
    }
    ...
    this.singleFlareTooltipProperty =
      options.singleFlareTooltipProperty || "name"; // The property name of the dataset to display in a tooltip for a flare when a flare represents a single object.
    if (options.clusterAreaDisplay) {
      this.clusterAreaDisplay =
        options.clusterAreaDisplay === "none"
          ? undefined
          : options.clusterAreaDisplay; // when to display the area (convex hull) of the points for each each cluster
    }
    ...
    this.refreshOnStationary =
      options.refreshOnStationary === false ? false : true; // whether this layer should refresh the clusters and redraw when stationary is true, default to true
    ...
    this.flareTextSymbol =
      options.flareTextSymbol ||
      new TextSymbol({
        color: new Color([255, 255, 255]),
        font: {
          size: 6,
    ...
    // initial data
    if (options.url) {
      this._createFromService(options.url).then(() => {
  ...
  private _createFromService(url: string): any {
    const dfd = new Deferer();
    let _self = this;
    ...
    esriRequest(queryIdsUrl, {
      ...
      var nums = 800;
      let promises = [];
      let queryByIdsUrl = url + "/query";
      while (ids.length > 0) {
        let subIds = ids.splice(0, nums);

        promises.push(
          esriRequest(queryByIdsUrl, {
            query: {
              f: "json",
              objectIds: subIds.join(",")
            },
            responseType: "json"
          })
        );
          ...
          function(res: any) {
            if (res && res.data && res.data.features) {
              ArrayUtil.forEach(
                res.data.features,
                function(v: any) {
                  data.push(lang.mixin(v.attributes, v.geometry));
                },
                this
              );
            }
  ...
  private _layerViewCreated(evt) {
    ...
    if (this.refreshOnStationary) {
      watchUtils.pausable(
        ...
        (isStationary, b, c, view) =>
          this._viewStationary(isStationary, b, c, view)
      );
    }
    ...
    if (evt.layerView.view.type === "2d") {
      ...
      watchUtils.whenTrueOnce(evt.layerView, "attached", () =>
  ...
  private _addViewEvents(layerView: any) {
    ...
    if (!v.fclPointerMove) {
      ...
      v.fclPointerMove = v.on("pointer-move", evt =>
        this._viewPointerMove(evt)
      );
      v.fclPointerDown = v.on("pointer-down", evt =>
        this._viewPointerMove(evt)
      );
    }
  }
  ...
  draw(activeView?: any) {
    ...
    if (this.clusterAreaDisplay && !this.areaRenderer) {
      console.error(
        "FlareClusterLayer: areaRenderer must be set if clusterAreaDisplay is set."
      );
      return;
    }
    ...
    let webExtent: any = !currentExtent.spatialReference.isWebMercator
      ? <Extent>(
          webMercatorUtils.project(
            currentExtent,
    ...
    for (let i = 0; i < dataLength; i++) {
      ...
      if (
        ...
        (web[1] <= webExtent.ymin || web[1] > webExtent.ymax)
      ) {
        continue;
      }
    ...
    if (this._isClustered) {
      for (let i = 0, len = this._gridClusters.length; i < len; i++) {
        if (this._gridClusters[i].clusterCount < this.clusterMinCount) {
          for (
            let j = 0, jlen = this._gridClusters[i].singles.length;
            j < jlen;
            j++
          ) {
  ...
  private _passesFilter(obj: any): boolean {
    ...
    for (let i = 0, len = this.filters.length; i < len; i++) {
      ...
      let valExists =
        filter.propertyValues.indexOf(obj[filter.propertyName]) !== -1;
      if (valExists) {
  ...
  private _createCluster(gridCluster: GridCluster) {
    ...
    let randomPointInfo = gridCluster.randomPoint;
    let randomPoint = new Point({
      x: randomPointInfo[this.xPropertyName],
      y: randomPointInfo[this.yPropertyName],
      z: randomPointInfo[this.zPropertyName]
    });

    if (!randomPoint.spatialReference.isWebMercator) {
      randomPoint = <Point>(
        webMercatorUtils.geographicToWebMercator(randomPoint)
      );
    }
    ...
    cluster.clusterGraphic.symbol =
      this.singleSymbol || this.clusterRenderer.defaultSymbol; // this.clusterRenderer.getClassBreakInfo(cluster.clusterGraphic).symbol;
    cluster.clusterGraphic.popupTemplate = this.singlePopupTemplate;
    ...
    if (
      ...
      if (area.rings && area.rings.length > 0) {
        ...
        if (!areaPoly.spatialReference.isWebMercator) {
          areaPoly = <Polygon>(
            webMercatorUtils.geographicToWebMercator(areaPoly)
          );
        }

        cluster.areaGraphic = new Graphic({
          geometry: areaPoly,
          attributes: areaAttr
        });
        cluster.areaGraphic.symbol = this.areaRenderer.getClassBreakInfo(
          cluster.areaGraphic
        ).symbol;
      }
    }
    ...
    if(this.labelField&&this.labelSymbol){
      ...
      let labelSymbol = this.labelSymbol.clone();
  ...
  private _createSurface() {
    if (
      this._activeView.fclSurface ||
      (this._activeView.type === "2d" && !this._layerView2d.container.element)
    )
      return;
    let surfaceParentElement = undefined;
    if (this._is2d) {
      surfaceParentElement =
        this._layerView2d.container.element.parentElement ||
        this._layerView2d.container.element.parentNode;
    } else {
      surfaceParentElement =
        this._activeView.canvas.parentElement ||
        this._activeView.canvas.parentNode;
    }

    let surface: any = gfx.createSurface(surfaceParentElement, "0", "0");
    surface.containerGroup = surface.createGroup();

    domStyle.set(surface.rawNode, {
      position: "absolute",
      top: "0",
      zIndex: -1
    });
  ...
  private _viewPointerMove(evt) {
    ...
    hitTest.then(response => {
      ...
      for (let i = 0, len = graphics.length; i < len; i++) {
        ...
        if (
          g &&
          (g.attributes.clusterId != null && !g.attributes.isClusterArea)
        ) {
  ...
  private _activateCluster(cluster: Cluster) {
    ...
    this.graphics.reorder(
      this._activeCluster.clusterGraphic,
      this.graphics.length - 1
    );
  ...
  private _initSurface() {
    ...
    let sp: ScreenPoint = this._activeView.toScreen(<Point>(
      this._activeCluster.clusterGraphic.geometry
    ));
  ...
  private _clearSurface() {
    let surface = this._activeView.fclSurface;
    query(".cluster-group", surface.containerGroup.rawNode).forEach(
  ...
  private _initCluster() {
    ...
    this._addClassToElement(
      this._activeCluster.clusterGroup.rawNode,
      "cluster-group"
    );
  ...
  private _initFlares() {
    ...
    let singleFlares =
      gridCluster.singles &&
      gridCluster.singles.length > 0 &&
      gridCluster.clusterCount <= this.maxSingleFlareCount;
    let subTypeFlares =
      !singleFlares &&
      (gridCluster.subTypeCounts && gridCluster.subTypeCounts.length > 0);
    ...
    let flareCount = willContainSummaryFlare
      ? this.maxFlareCount
      : flares.length;
    ...
    let clusterScreenPoint = this._activeView.toScreen(<Point>(
      this._activeCluster.clusterGraphic.geometry
    ));
    let clusterSymbolSize = <number>(
      this._activeCluster.clusterGraphic.symbol.get("size")
    );
    for (let i = 0; i < flareCount; i++) {
      ...
      let isSummaryFlare =
        willContainSummaryFlare && i >= this.maxFlareCount - 1;
      if (isSummaryFlare) {
        ...
        for (
          let j = this.maxFlareCount - 1, jlen = flares.length;
          j < jlen;
          j++
        ) {
    ...
    for (let i = 0, len = flares.length; i < len; i++) {
      ...
      f.flareGroup.mouseEnter = on.pausable(
        f.flareGroup.rawNode,
        "mouseenter",
        () => this._createTooltip(f)
      );
      f.flareGroup.mouseLeave = on.pausable(
        f.flareGroup.rawNode,
        "mouseleave",
  ...
  private _setFlarePosition(
    flareGroup: any,
    clusterSymbolSize: number,
    flareCount: number,
    flareIndex: number,
  ...
  private _getFlareSymbol(flareGraphic: Graphic): SimpleMarkerSymbol {
    return !this.flareRenderer
      ? this.flareSymbol
      : this.flareRenderer.getClassBreakInfo(flareGraphic).symbol;
  }
  ...
  private _createTooltip(flare: Flare) {
    ...
    for (let i = 0, len = lines.length; i < len; i++) {
      let textShape = tooltipGroup
        ...
        .setFont({
          size: 10,
          family: this.flareTextSymbol.font.get("family"),
  ...
  private _destroyTooltip() {
    query(".tooltip-text", this._activeView.fclSurface.rawNode).forEach(
      domConstruct.destroy
    );
  }
  ...
  private _removeClassFromElement(
    ...
    let removeClass: Function = (_element, _className) => {
      ...
      _element.setAttribute(
        "class",
        currentClass.replace(" " + _className, "")
      );
    };
...
export class PointFilter {
  ...
  constructor(
    ...
    propertyName: string,
    values: any[],
    keepOnlyIfValueExists: boolean = false
  ) {
