...
define(["require", "exports", "esri/layers/GraphicsLayer", "esri/symbols/SimpleMarkerSymbol", "esri/symbols/TextSymbol", "esri/symbols/SimpleLineSymbol", "esri/Color", "esri/core/watchUtils", "esri/geometry/support/webMercatorUtils", "esri/Graphic", "esri/geometry/Point", "esri/geometry/Multipoint", "esri/geometry/Polygon", "esri/geometry/geometryEngine", "esri/geometry/SpatialReference", "esri/core/accessorSupport/decorators", "dojo/on", "dojox/gfx", "dojo/dom-construct", "dojo/query", "dojo/dom-attr", "dojo/dom-style", "dojo/_base/Deferred", "esri/request", "dojo/_base/array", "dojo/promise/all", "dojo/_base/lang"], function (require, exports, GraphicsLayer, SimpleMarkerSymbol, TextSymbol, SimpleLineSymbol, Color, watchUtils, webMercatorUtils, Graphic, Point, Multipoint, Polygon, geometryEngine, SpatialReference, asd, on, gfx, domConstruct, query, domAttr, domStyle, Deferer, esriRequest, ArrayUtil, all, lang) {
    ...
    var FlareClusterLayer = /** @class */ (function (_super) {
        ...
        FlareClusterLayer.prototype._createFromService = function (url) {
            var _this = this;
            var dfd = new Deferer();
            var _self = this;
            var queryIdsUrl = url +
            ...
            esriRequest(queryIdsUrl, {
                ...
                var nums = 800;
                var promises = [];
                var queryByIdsUrl = url + "/query";
                while (ids.length > 0) {
                    var subIds = ids.splice(0, nums);
                    promises.push(esriRequest(queryByIdsUrl, {
                        query: {
                            f: "json",
                ...
                all(promises).then(function (reses) {
                    console.log(reses);
                    var data = [];
                    ArrayUtil.forEach(reses, function (res) {
                        if (res && res.data && res.data.features) {
                            ArrayUtil.forEach(res.data.features, function (v) {
        ...
        FlareClusterLayer.prototype._layerViewCreated = function (evt) {
            ...
            if (this.refreshOnStationary) {
                watchUtils.pausable(evt.layerView.view, "stationary", function (isStationary, b, c, view) {
            ...
            if (evt.layerView.view.type === "2d") {
                ...
                watchUtils.whenTrueOnce(evt.layerView, "attached", function () {
                    return _this._addViewEvents(evt.layerView);
                });
            }
            else {
        ...
        FlareClusterLayer.prototype.draw = function (activeView) {
            ...
            var webExtent = !currentExtent.spatialReference.isWebMercator
                ? (webMercatorUtils.project(currentExtent, new SpatialReference({ wkid: 102100 })))
                : currentExtent;
        ...
        FlareClusterLayer.prototype._createCluster = function (gridCluster) {
            ...
            var randomPointInfo = gridCluster.randomPoint;
            ...
            if (!randomPoint.spatialReference.isWebMercator) {
                randomPoint = (webMercatorUtils.geographicToWebMercator(randomPoint));
            }
            // let attributes: any = {
            //   x: gridCluster.x,
            //   y: gridCluster.y,
        ...
        FlareClusterLayer.prototype._initFlares = function () {
            ...
            var flareCount = willContainSummaryFlare
                ? this.maxFlareCount
                : flares.length;
